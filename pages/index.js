...
<\/script>
                ...
                {!isMobile&&<div style={{position:'relative'}}>
                  ...
                  {showPrintMenu&&<div style={{position:'absolute',top:'100%',right:0,marginTop:'6px',background:'#fff',border:'1px solid #e0e0e0',borderRadius:'12px',boxShadow:'0 8px 24px rgba(0,0,0,0.12)',zIndex:20,minWidth:'240px',overflow:'hidden'}}>
                    ...
                    <div style={{padding:'6px 16px 4px',fontSize:'10px',fontWeight:'700',color:'#aaa',textTransform:'uppercase',letterSpacing:'0.08em'}}>Κείμενο + Ερωτήσεις</div>
                    {/* 2. Πρόχειρη — μία σελίδα */}
                    <button onClick={()=>{
                      const qs=fileQuestions(modalFile.id);
                      const qHtml=qs.length>0?[...qs].sort((a,b)=>sortCode(a.code)-sortCode(b.code)).map((q,i)=>`<tr><td style="padding:6px 12px;border:1px solid #ddd;font-weight:600;text-align:center;width:60px;font-size:13px">${q.code||i+1}</td><td style="padding:6px 12px;border:1px solid #ddd;font-size:13px;line-height:1.6">${q.text}</td></tr>`).join(''):'<tr><td colspan="2" style="padding:12px;color:#888;font-style:italic;font-size:13px">Δεν υπάρχουν ερωτήσεις</td></tr>';
                      const w=window.open('about:blank','_blank');
                      const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${modalFile.title} — Πρόχειρη εκτύπωση</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:Georgia,serif;} .page{max-width:800px;margin:0 auto;padding:40px 32px;} h1{font-size:18px;margin-bottom:4px;color:#1a1a1a;} .sub{font-size:12px;color:#888;margin-bottom:24px;} .pdf-frame{width:100%;height:65vh;border:1px solid #ddd;border-radius:8px;margin-bottom:24px;} h2{font-size:15px;color:#1a1a1a;margin-bottom:12px;border-bottom:2px solid #e8dfc4;padding-bottom:6px;} table{width:100%;border-collapse:collapse;margin-bottom:24px;} #loading{text-align:center;padding:40px;color:#888;font-size:14px;} @media print{.no-print{display:none!important;} .pdf-frame{height:auto;min-height:60vh;}}</style></head><body><div class="page"><h1>${modalFile.title}</h1><p class="sub">${modalFile.name}</p><div id="loading">⏳ Φόρτωση κειμένου…</div><iframe class="pdf-frame" src="/api/files/pdf/${modalFile.id}" onload="document.getElementById('loading').style.display='none';document.getElementById('actions').style.display='flex';"></iframe><h2>Ερωτήσεις</h2><table>${qHtml}</table><div id="actions" class="no-print" style="display:none;gap:10px;margin-top:12px"><button onclick="window.print()" style="background:#1a1a1a;color:#fff;border:none;padding:10px 24px;border-radius:10px;font-size:13px;cursor:pointer">🖨️ Εκτύπωση</button></div></div></body></html>`;
                      w.document.open();
                      w.document.write(html);
                      w.document.close();
                      ...
                      onMouseEnter={e=>e.currentTarget.style.background='#faf6ea'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <span>📋</span><div><div style={{fontWeight:'600'}}>Πρόχειρη</div><div style={{fontSize:'11px',color:'#888',marginTop:'2px'}}>Κείμενο και ερωτήσεις σε μία σελίδα</div></div>
                    </button>
                    {/* 3. Κανονική — ξεχωριστές σελίδες */}
                    <button onClick={()=>{
                      const qs=fileQuestions(modalFile.id);
                      ...
                      const w=window.open('about:blank','_blank');
                      const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${modalFile.title} — Κανονική εκτύπωση</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11pt;line-height:1.2;color:#1a1a1a;} .page{max-width:800px;margin:0 auto;padding:40px 32px;} h1{font-size:14pt;font-weight:700;margin-bottom:4px;} .sub{font-size:9pt;color:#888;margin-bottom:24px;} .pdf-frame{width:100%;height:85vh;border:1px solid #ddd;border-radius:8px;} .questions-page{page-break-before:always;padding-top:40px;} h2{font-size:12pt;font-weight:700;margin-bottom:8px;border-bottom:1.5px solid #c0c0c0;padding-bottom:6px;} .qtitle{font-size:9pt;color:#888;margin-bottom:16px;} .q-item{margin-bottom:2em;font-size:11pt;line-height:1.2;} .q-code{font-weight:700;} #loading{text-align:center;padding:60px;color:#888;font-size:12pt;} @media print{.no-print{display:none!important;} .pdf-frame{height:auto;min-height:85vh;} .questions-page{page-break-before:always;}}</style></head><body><div class="page"><h1>${modalFile.title}</h1><p class="sub">${modalFile.name}</p><div id="loading">⏳ Φόρτωση κειμένου…</div><iframe class="pdf-frame" src="/api/files/pdf/${modalFile.id}" onload="document.getElementById('loading').style.display='none';document.getElementById('actions').style.display='flex';"></iframe><div class="questions-page"><h2>Ερωτήσεις</h2><p class="qtitle">${modalFile.title}</p>${qHtml}</div><div id="actions" class="no-print" style="display:none;gap:10px;margin-top:24px"><button onclick="window.print()" style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background:#1a1a1a;color:#fff;border:none;padding:10px 24px;border-radius:10px;font-size:11pt;cursor:pointer">🖨️ Εκτύπωση</button></div></div></body></html>`;
                      w.document.open();
                      w.document.write(html);
                      w.document.close();
                      setShowPrintMenu(false);
                      ...
                      onMouseEnter={e=>e.currentTarget.style.background='#faf6ea'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <span>📝</span><div><div style={{fontWeight:'600'}}>Κανονική</div><div style={{fontSize:'11px',color:'#888',marginTop:'2px'}}>Ερωτήσεις σε ξεχωριστή σελίδα</div></div>
                    </button>
                  </div>}
                </div>}
