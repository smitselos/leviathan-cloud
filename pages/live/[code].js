...
export default function LivePresentation() {
  ...
  const [activeTab, setActiveTab] = useState('pdf'); // 'pdf' | 'app'
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#000',display:'flex',flexDirection:'column'}}>
      ...
      {hasApp && (
        <div style={{display:'flex',background:'#1a1a1a',flexShrink:0,height:'44px'}}>
          ...
          <button onClick={()=>setActiveTab('split')}
            style={{width:'60px',background:'transparent',border:'none',borderBottom:activeTab==='split'?'2px solid #e8c96a':'2px solid transparent',color:activeTab==='split'?'#e8c96a':'#8e8ea0',fontSize:'18px',cursor:'pointer'}}>
            ⊞
          </button>
        </div>
      )}
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        {/* Εφαρμογή */}
        {hasApp&&(activeTab==='app'||activeTab==='split')&&(
          <iframe src={session.appSrc}
            style={{flex:1,border:'none',height:'100%'}}
