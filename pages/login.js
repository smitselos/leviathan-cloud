      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'stretch', flexWrap: 'wrap', justifyContent: 'center', maxWidth: 700, width: '100%' }}>
        ...
        <div style={{ ...S.card, flex: 1, minWidth: 280, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <p style={{ fontSize: 13, color: '#6b6b80', marginBottom: 16, lineHeight: 1.6 }}>
            Γράψε το όνομα του εκπαιδευτικού (όπως στο email του, πριν το @) για να δεις το δημοσιευμένο υλικό του — χωρίς σύνδεση.
          </p>
          <input
            value={teacherName}
            onChange={(e) => setTeacherName(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && teacherName.trim()) window.open('/s/' + encodeURIComponent(teacherName.trim()), '_blank'); }}
            placeholder="π.χ. smitselos ή email εκπαιδευτικού"
