...
export async function getFileContent(accessToken, fileId) {
  ...
  try {
    ...
    const exportMime = GOOGLE_EXPORT_MAP[mime];

    if (exportMime) {
      // Native Google format → export σε PDF
      ...
      return { data: response.data, mimeType: exportMime };
    } else {
      // Κανονικό αρχείο (PDF, DOCX, PPTX κ.λπ.) → download
